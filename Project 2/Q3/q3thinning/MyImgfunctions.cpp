...
int MorphThin(unsigned char *ImageData, unsigned char *ImageDataOp, int ImgHeight, int ImgWidth)
{
    ...
    for(int i=0;i<ImgHeight;i++)
    {
        for(int j=0;j<ImgWidth;j++)
        {
            if(PaddedImage[(i+N)*ImgWidthNew+j+N] == 255)
            {
                ...
                int hit = CheckCondPattern(pattern,'T');
                if(hit==1)
    ...
    for(int i=0;i<ImgHeight;i++)
    {
        for(int j=0; j<ImgWidth; j++)
        {
            if(Temp[i*ImgWidth+j]==255)
            {
                ...
                int hit = CheckUncondPattern(pattern,'T');
                if(hit==1)
