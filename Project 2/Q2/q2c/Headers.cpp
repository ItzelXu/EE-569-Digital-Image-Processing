...
double*** Convert1dto3d(double* Imagedata, int ImgHeight, int ImgWidth, int BytesPerPixel)
{
	double*** Imagedata3d =  new double **[ImgHeight]();
	for (int i = 0; i < ImgHeight; i++)
	{
		Imagedata3d[i] = new double *[ImgWidth]();
		for (int j = 0; j < ImgWidth; j++)
		{
			Imagedata3d[i][j] = new double[BytesPerPixel]();
...
double* Convert3dTo1d(double*** Imagedata3d, int ImgHeight, int ImgWidth, int BytesPerPixel)
{
	double* Imagedata = new double[ImgHeight*ImgWidth*BytesPerPixel]();
	int m = 0;
	for (int i = 0; i < ImgHeight; i++)
	{
		for (int j = 0; j < ImgWidth; j++)
...
unsigned char* ConvertGreyToBinary (unsigned char*ImageData, int ImgHeight, int ImgWidth)
{
    ...
    cout<<"Image has been successfully converted to binary!"<<endl;
    return TempOp;
}

unsigned char*** Convert1dto3dus(unsigned char* Imagedata, int ImgHeight, int ImgWidth, int BytesPerPixel)
{
...
unsigned char* Convert3dTo1dus(unsigned char*** Imagedata3d, int ImgHeight, int ImgWidth, int BytesPerPixel)
{
